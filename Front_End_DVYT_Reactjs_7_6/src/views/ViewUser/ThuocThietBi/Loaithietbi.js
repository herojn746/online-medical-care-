...
function Loaithietbi() {
  ...
  const { idLoaiThietBi } = useParams(); 
  console.log("Received ID from useParams:", idLoaiThietBi);
  const navigate = useNavigate();

  useEffect(() => {
    const a = parseInt(idLoaiThietBi);
    console.log(a);
    axios.get(`http://localhost:5199/api/ThietBiYTe/get-all-thiet-bi-by-ma-loai?maLoaiThietBi=${idLoaiThietBi}`)
      .then(response => {
  ...
  if (thietBi.length === 0) {
    return <div>No devices found for this category.</div>;
  }
        ...
        {thietBi.map(item => (
          ...
          <Col lg="4" md="6" sm="12" key={item.id}>
            ...
            <Card className="fixed-size-card">
              ...
              />
              <CardBody>
                <CardTitle tag="h5">{item.tenThietBiYTe}</CardTitle>
