...
import bannerThietBi from "assets/img/yte1.jpg";
import { useNavigate } from "react-router-dom";

function DeviceList() {
  const [thietBi, setThietBi] = useState([]);
  const [filteredThietBi, setFilteredThietBi] = useState([]);
  ...
  useEffect(() => {
    axios.get("http://localhost:5199/api/ThietBiYTe/get-all-thiet-bi-y-te")
      .then(response => {
        setThietBi(response.data);
        setFilteredThietBi(response.data);
        setLoading(false);
      })
      .catch(error => {
  ...
  const handleSearch = (event) => {
    ...
    setSearchKey(key);
    const filtered = thietBi.filter(item =>
      item.tenThietBiYTe.toLowerCase().includes(key) ||
      item.donViTinh.toLowerCase().includes(key) ||
      item.moTa.toLowerCase().includes(key)
    );
    setFilteredThietBi(filtered);
  };
  ...
  return (
    <div className="container mt-5">
      <div className="banner">
        <img src={bannerThietBi} alt="Banner" />
          ...
          <div className="col-equal-height4" key={item.id}>
            <Card className="card4">
              ...
              <CardBody className="card-body4" onClick={() => handleItemDetail(item)}>
                <CardTitle className="card-title4">{item.tenThietBiYTe}</CardTitle>
      ...
      {selectedItem && (
        <Modal isOpen={!!selectedItem} toggle={() => setSelectedItem(null)}>
          ...
          <ModalBody>
            <img src={`${selectedItem.hinhAnh.startsWith("/") ? `http://localhost:5199${selectedItem.hinhAnh}` : selectedItem.hinhAnh}`} alt={selectedItem.tenThietBiYTe} style={{ width: '100%' }} />
            <h5>{selectedItem.tenThietBiYTe}</h5>
